...
- 1. 基于 token 验证与存储实现登录与信息的初始化
- 2. 将 token 与 fd 绑定，token 与 用户信息绑定，即可实现通过用户信息，查看用户是否在线等状态判定
- 3. 当用户登录成功，生成并返回给用户 token，此后的所有请求，都带着此 token 来证明身份
- 4. 聊天页面需要执行 init() 进行缓存的初始化，保存用户信息，token，以及 fd 的绑定，像所有好友推送上线消息
- 5. 聊天，加好友，处理好友请求等操作，都以固定格式，推送消息到服务器，服务器转到相应 websocket 控制器去处理，返回结果
- 5. 当用户关闭页面触发 onclose ，对缓存进行销毁
